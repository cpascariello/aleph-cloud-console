...
import { EntityType } from 'aleph-sdk'
...
export function ConfidentialTab() {
  ...
  const confidentialInstances = allInstances.filter(
    (inst) => (inst as { type: string }).type === EntityType.Confidential,
  )

  const list = useResourceList<Instance>({
    items: confidentialInstances,
    getId: (item) => item.id,
    searchFn: (item, query) =>
  ...
  if (list.isEmpty) {
    return (
      <ResourceEmptyState
        resourceName="Confidential Instance"
        createHref="/compute/new"
        createLabel="Deploy Confidential Instance"
      />
    )
  }
  ...
  const rows: RowShape[] = list.paginatedItems.map((inst) => ({
    ...
    tee: <Badge variant="info">TEE</Badge>,
    date: (
